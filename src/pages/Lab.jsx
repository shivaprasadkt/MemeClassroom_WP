  ...
  return (
    <div className="max-w-6xl mx-auto py-8 px-4" onPointerMove={handlePointerMove} onPointerUp={handlePointerUp}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {/* Left 2 Columns: Media Tabs & Interactive Workspace */}
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* Active Canvas Workspace Bounding Container */}
          <div 
            ref={canvasContainerRef}
            className="relative w-full max-w-lg mx-auto bg-gray-150 dark:bg-gray-900 border border-gray-300 dark:border-gray-800 rounded-xl overflow-hidden aspect-square flex items-center justify-center select-none"
          >
              ...
              <div className={`w-full h-full grid gap-1 ${
                ...
                {images.length > 0 ? (
                  ...
                  <div className="flex flex-col items-center justify-center p-8 text-center text-gray-400 w-full h-full">
                    ...
                    <p className="text-xs">Upload images or select presets below to build a collage grid.</p>
                  </div>
                )}
              </div>
            )}
          ...
          {/* Media Specific Sub-Controls Panel */}
          <div className={`p-6 ${containerClass}`}>
            
            {activeTab === "image" && (
              <div className="space-y-4">
                <h3 className="font-bold text-sm border-b pb-2 mb-4 uppercase tracking-wider">Collage Controls</h3>
                <div className="flex flex-col sm:flex-row gap-4">
                  <label className="flex-grow">
                    <span className="block text-xs font-semibold uppercase tracking-wider mb-2">Upload layout pictures (Max 4)</span>
                    <input 
                      type="file" 
                      multiple 
                      accept="image/*" 
                  ...
                  {images.length > 0 && (
                    <button 
                      onClick={() => setImages([])}
                      className="text-xs font-semibold text-red-650 hover:underline self-end"
                    >
                      Clear Collage Grid
                    </button>
                  )}
            ...
            {activeTab === "video" && (
              <div className="space-y-4">
                <h3 className="font-bold text-sm border-b pb-2 mb-4 uppercase tracking-wider">Video Controls</h3>
                
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-semibold uppercase tracking-wider mb-2">Upload Video (&lt; 15s)</label>
                    <input 
                      type="file" 
                      accept="video/*" 
                  ...
                  <div>
                    <span className="block text-xs font-semibold uppercase tracking-wider mb-2">Load Mock Sample</span>
                    <div className="flex gap-2">
                      {MEDIA_SAMPLES.video.map((sample, idx) => (
                        <button
                          key={sample.id}
                          onClick={() => selectMediaPreset(sample.url, "video", 15)}
                          className="text-xs bg-purple-50 dark:bg-purple-950/20 text-purple-750 dark:text-purple-300 font-bold px-3 py-1.5 rounded-lg border border-purple-200 dark:border-purple-800/40 hover:bg-purple-100"
                        >
                          Sample {idx + 1}
                        </button>
                      ))}
                ...
                {/* Range Trim Slider stubs */}
                {videoUrl && (
                  <div className="pt-4 border-t border-gray-100 dark:border-gray-700">
                    <span className="block text-xs font-semibold uppercase tracking-wider mb-3">Crop / Trim Playback Window</span>
                    <div className="grid grid-cols-2 gap-4 text-xs font-semibold">
                      <div>
                        <label>Start Timestamp: {videoTrimStart.toFixed(1)}s</label>
                        <input 
                          type="range" 
                          min="0" 
                          max={videoTrimEnd} 
                      ...
                      <div>
                        <label>End Timestamp: {videoTrimEnd.toFixed(1)}s</label>
                        <input 
                          type="range" 
                          min={videoTrimStart} 
                          max={videoDuration} 
            ...
            {activeTab === "gif" && (
              <div className="space-y-4">
                <h3 className="font-bold text-sm border-b pb-2 mb-4 uppercase tracking-wider">GIF Controls</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-semibold uppercase tracking-wider mb-2">GIF Source URL</label>
                    <input 
                      type="text" 
                      value={gifUrl} 
                      onChange={(e) => setGifUrl(e.target.value)} 
                      placeholder="Giphy or upload URL"
                      className="w-full px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-lg text-xs bg-gray-50 dark:bg-gray-900"
                    />
                  </div>
                  <div>
                    <span className="block text-xs font-semibold uppercase tracking-wider mb-2">Load Mock Sample</span>
            ...
            {activeTab === "audio" && (
              <div className="space-y-4">
                <h3 className="font-bold text-sm border-b pb-2 mb-4 uppercase tracking-wider">Audio Controls</h3>
                
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-semibold uppercase tracking-wider mb-2">Upload Audio (&lt; 20 MB)</label>
                    <input 
                      type="file" 
                      accept="audio/*" 
                      onChange={(e) => {
                ...
                {audioUrl && (
                  <div className="pt-4 border-t border-gray-100 dark:border-gray-700">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-xs font-semibold">
                      <div>
                        <label>Start Timestamp: {audioTrimStart.toFixed(1)}s</label>
                        <input 
                          type="range" 
                          min="0" 
                          max={audioTrimEnd} 
                      ...
                      <div>
                        <label>End Timestamp: {audioTrimEnd.toFixed(1)}s</label>
                        <input 
                          type="range" 
                          min={audioTrimStart} 
                          max="45" 
        ...
        {/* Right 1 Column: Text Overlay Editor Panel & Template Submissions */}
        <div className="space-y-6">
          
          {/* Overlay Text Engine Settings Card */}
          <div className={`p-6 ${containerClass}`}>
            <h2 className="text-md font-bold mb-4 uppercase tracking-wider border-b pb-2">Text Styling Controls</h2>
            
            <button
              onClick={addTextLayer}
              className="w-full bg-purple-100 text-purple-750 dark:bg-purple-950 dark:text-purple-300 border border-purple-250 dark:border-purple-800 hover:bg-purple-200 font-semibold py-2 px-4 rounded-lg shadow-sm text-xs transition duration-200 mb-6"
            >
            ...
            {activeTextLayer ? (
              <div className="space-y-4 text-xs font-semibold">
                <div>
                  <label className="block text-gray-500 uppercase mb-1">Selected Text ID</label>
                  <input
                    type="text"
                    value={activeTextLayer.id}
                    disabled
                    className="w-full px-2 py-1 bg-gray-100 border rounded cursor-not-allowed"
                  />
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-2 py-1 border border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 rounded"
                    >
              ...
              <p className="text-xs text-gray-400 text-center py-4">Click a text overlay on the canvas to configure settings.</p>
            )}
          </div>

          {/* Unified Save & Download Button */}
          <button
            onClick={() => setShowSaveModal(true)}
            className="w-full bg-indigo-600 hover:bg-indigo-750 text-white font-bold py-3 px-4 rounded-xl shadow-md transition duration-200 text-sm"
          >
            💾 Save & Download Meme
          </button>

          {/* Separate Template Contribution Form pipeline */}
          <div className={`p-6 ${containerClass}`}>
            <h2 className="text-md font-bold mb-4 uppercase tracking-wider border-b pb-2">Contribute Blank Template</h2>
            <form onSubmit={handleTemplateUploadSubmit} className="space-y-4 text-xs font-semibold">
              
              {templateSuccess && (
                <div className="p-3 bg-purple-50 dark:bg-purple-950/20 text-purple-750 rounded-lg border text-[11px]">
                  {templateSuccess}
                </div>
              )}
